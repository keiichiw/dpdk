...
RTE_LOG_REGISTER(virtio_vhost_user_logtype, driver.virtio.vhost_user, NOTICE);
#define DRV_LOG(level, fmt, args...)                                    \
	rte_log(RTE_LOG_##level, virtio_vhost_user_logtype, "virtio-vhost-iuser %s(): " fmt "\n", \
		__func__, ##args)
...
static void vvu_process_new_connection(struct vvu_socket *vvu_socket)
{
	...
	DRV_LOG(INFO, "new device, handle is %d\n", dev->vid);
...
static void vvu_process_status_change(struct vvu_socket *vvu_socket, bool slave_up,
				      bool master_up)
{
	DRV_LOG(DEBUG, "%s slave_up %d master_up %d\n",
...
vvu_interrupt_handler(void *cb_arg)
{
	...
	if (isr & VIRTIO_PCI_ISR_CONFIG) {
		...
		DRV_LOG(DEBUG, "%s isr %#x status %#x\n", __func__, isr, status);
...
vvu_virtio_pci_init_vq(struct vvu_socket *vvu_socket, int vq_idx)
{
	...
	DRV_LOG(DEBUG, "vq %d q_num: %u\n", vq_idx, q_num);
	if (q_num == 0) {
		DRV_LOG(ERR, "virtqueue %d does not exist\n",
			vq_idx);
		return -1;
	}
...
vvu_virtio_pci_init_bar(struct vvu_socket *vvu_socket)
{
	...
	if (!vvu_socket->doorbells) {
		DRV_LOG(ERR, "BAR 2 not available\n");
		return -1;
	}
